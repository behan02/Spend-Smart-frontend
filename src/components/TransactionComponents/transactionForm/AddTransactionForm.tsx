...
const AddTransactionForm: React.FC<TransactionFormProps> = ({
    ...
    async function fetchCategories() {
      try {
        const response = await fetch(
          `https://localhost:7211/api/Category/GetCategories?type=${type}`
        );
        if (!response.ok) {
  ...
  async function handleSubmit(e: React.FormEvent<HTMLFormElement>) {
    ...
    try {
      const response = await fetch(
        `https://localhost:7211/api/Transaction/CreateTransaction/${userId}`,
        {
          method: "POST",
          headers: {
              ...
              <FormControl size="small" fullWidth required>
                ...
                >
                  {categories.map((category) => {
                    const iconObj = CategoryIcons.find(
                      (icon) => icon.category === category.name
                    );
                    return (
